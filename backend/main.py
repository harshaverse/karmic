...
def optimize_mesh_to_outer_shell(input_path: str, output_path: str) -> bool:
    ...
    This replicates the functionality from the original Python script.
    """
    try:
        # Load the mesh
        ...
        # Remove internal faces and create outer shell
        # Method 1: Voxelize and reconstruct (similar to original script)
        voxel_size = max(mesh.extents) / 100  # Adaptive voxel size
        
        # Create voxel grid
        voxel_grid = mesh.voxelized(pitch=voxel_size)
        
        # Convert back to mesh (this creates the outer shell)
        outer_shell = voxel_grid.as_boxes()
        
        # Alternative method: Use convex hull for simpler outer shell
        if outer_shell.vertices.shape[0] == 0:
            outer_shell = mesh.convex_hull
        
        # Simplify the mesh to reduce complexity
        if len(outer_shell.faces) > 10000:
            outer_shell = outer_shell.simplify_quadric_decimation(face_count=5000)
        ...
        print(f"Error optimizing mesh: {str(e)}")
        return False
